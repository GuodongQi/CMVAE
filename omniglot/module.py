...
class CausalDecoder_(nn.Module):
    def __init__(self, latent_size):
        super().__init__()
        self.mean_mlp = MLP(latent_size, 2 * latent_size, latent_size)
        self.logvar_mlp = MLP(latent_size, 2 * latent_size, latent_size)

        self.I = torch.eye(latent_size).cuda()

    def cal_mean(self, x, A, u):
        x = (x + u).bmm(torch.inverse(self.I - A).unsqueeze(0).repeat(x.shape[0], 1, 1))
        x = self.mean_mlp(x)
        return x

    def cal_logvar(self, x, A, u):
        x = (x + u.var(1, keepdim=True).repeat(1, u.shape[1], 1)).bmm(
            torch.inverse(self.I - A).pow(2).unsqueeze(0).repeat(x.shape[0], 1, 1))
        x = x.log()
        x = self.logvar_mlp(x)
        return x
...
class CausalEncoder(nn.Module):
    def __init__(self, latent_size, bias=True, linear=True):
        ...
        if linear:
            self.I = torch.eye(latent_size).cuda()
            self.A = nn.Parameter(torch.zeros([self.latent_size, self.latent_size]).cuda(),
                                  requires_grad=True)
        else:
...
class CausalDecoder(nn.Module):
    def __init__(self, latent_size):
        super().__init__()
        self.mlp = MLP(latent_size, 2 * latent_size, latent_size)
        self.I = torch.eye(latent_size).cuda()

    def forward(self, x, A, u):
        # x = self.mlp(x+u)
        # x = x.bmm(torch.inverse(self.I - A).unsqueeze(0).repeat(x.shape[0], 1, 1))
        # x = (x + u).bmm(torch.inverse(self.I - A).unsqueeze(0).repeat(x.shape[0], 1, 1))
        # x = self.mlp(x)
        return x
...
class MLP(nn.Module):
    def __init__(self, in_dim, latent_dim, output_dim):
