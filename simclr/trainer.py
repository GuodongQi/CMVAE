...
from module import MimgnetEncoder, CelebrEncoder
from data import MimgNetDataset, CelebADataset
...
class Trainer(object):
    def __init__(self, args):
        ...
        if self.args.dataset == "mini":
            ...
            dataset = CelebADataset(os.path.join(self.args.data_dir), mode='train', simclr=True)
            self.encoder = CelebrEncoder(
                hidden_size=args.hidden_size
            ).to(args.device)
    ...
    def save_feature(self):
        ...
        for mode in ['train', 'val', 'test']:
            if self.args.dataset == "min":
                dataset = MimgNetDataset(os.path.join(self.args.data_dir), mode=mode)
            elif self.args.dataset == "celeba":
                dataset = CelebADataset(os.path.join(self.args.data_dir),mode=mode)
